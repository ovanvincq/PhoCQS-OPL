...
# ╔═╡ e586af70-1db5-11ef-2abe-1f8b38d28b6c
using OpticalFibers, Gridap,GridapGmsh,PlutoUI

# ╔═╡ 8f8bd5b0-583b-4bff-b109-ecbbb627e0d4
using OpticalFibers.ModeSolvers, GridapMakie, GLMakie

# ╔═╡ 410b6956-2819-493e-ae19-f82472aa9508
md"""
# Coupling and propagation
As an example, we will study the case of a parabolic core fiber. The radius of the coreis 5 µm and its maximum refractive index is 1.48. The cladding is homogeneous and its refractive index is 1.45.
"""

# ╔═╡ 5540beed-102f-46a7-bc65-29bd33b0138e
md"## Calculation of guided modes in the saclar approximation"

# ╔═╡ 6813467b-df42-4b53-968e-60ef86df082b
begin
	ng=1.45
	nc=1.48
	lambda=1
...
# ╔═╡ b7d0199a-8a3d-4595-b866-9d070dc3d644
md"In 1D"

# ╔═╡ 1fb7f90f-cc42-4bf9-a02a-a6eabb18b7a1
epsilon=x->(ng+(nc-ng)*(x[1]<=5)*(1-x[1]^2/5^2))^2;

# ╔═╡ 6ed88631-0aba-46e7-b334-514443e264e7
model = CartesianDiscreteModel((0,25),1000);

# ╔═╡ 61f2f7cf-46ff-4aea-a21e-4cd51b456627
ms=FEM1D.(lambda,[0,1,2,3,4],epsilon,Ref(model),neigs=4,field=true)

# ╔═╡ e6d4d5bd-52f2-4931-90e6-0c3af8ea40da
md"In 2D"

# ╔═╡ 508f2a99-4c53-439a-b220-2953db02c6e3
epsilon2D=x->epsilon(hypot(x[1],x[2]));

# ╔═╡ 5d79eb3b-547e-4363-8ba2-d64257b705fc
model2D = GmshDiscreteModel("example1_LMA.msh");

# ╔═╡ ea17de0a-9f54-47ff-90ce-227c67f20375
wireframe(Triangulation(model2D),axis=(aspect=DataAspect(),))

# ╔═╡ 87860d74-4130-425f-b6f7-8bf0a614c0ad
ms2D=FEM2D(lambda,epsilon2D,model2D,field=true,neigs=20)

# ╔═╡ 4bde8644-d3ef-42e2-8bb5-aa477f8a2f82
md"The number of modes is the same in 1D as in 2D"

# ╔═╡ bb15a924-e379-47d6-9e0a-8ad4a3cdc185
@bind nmode NumberField(1:length(ms2D),default=1)

# ╔═╡ 63d43a44-9bd5-4e38-89d6-9391295819fa
plot(real(ms2D[nmode].field.E),axis=(aspect=DataAspect(),title="Re(E)"))

# ╔═╡ 3feaf821-2d4d-4e6a-ba8a-8964052c0302
md"## Mode normalization"

# ╔═╡ 8e64314e-8cfa-43b0-9357-3c01ae2b036c
md"""
In the scalar approximaiton, the overlap integral between two modes is defined by: $\iint E_1 E_2^* dS$.
"""

# ╔═╡ 6f1b8944-367c-467f-8a97-40e0defdcc2c
md"Modes given by mode solvers are not normalized:"

# ╔═╡ 6afef2d2-2bcc-42ed-86a0-90640bc0d41d
overlap.(ms2D,transpose(ms2D))

# ╔═╡ c1451fd4-97de-4dee-a3ce-dbd0c1907e1b
md"But they can be normized with the help on the function `normalize!`"

# ╔═╡ 2cc90920-1f21-4051-b69b-43585939b8ea
normalize!.(ms2D);

# ╔═╡ c0b3b403-63b3-4bfc-bab8-4bfd199b59d4
overlap.(ms2D,transpose(ms2D))

# ╔═╡ 135d18b6-b5b8-4b4e-9efc-cdc4f22ceabe
md"""
## Coupling in the fiber
A gaussian beam whith a waist `wf` is injected in the fiber.
"""

# ╔═╡ d158559c-4aec-49d5-9385-f2e417ea007d
wf=1;

# ╔═╡ 853c18f9-b70e-4f04-a45c-42c472ced6fe
md"First, we define the injected beam"

# ╔═╡ 40587dfa-c924-45ce-bdec-7c8b462276e8
f=ScalarFieldFunction2D(x->exp(-hypot(x[1],x[2])^2/wf^2)/sqrt(pi/2*wf^2));

# ╔═╡ 5d2b7f3b-1bbd-40e2-9939-d5c8a9f1f534
md"We can verify that its field in normalized"

# ╔═╡ a210296a-bb34-4e40-b394-9d9691ac11fa
overlap(f,f)

# ╔═╡ 16558d1e-901a-4eae-abe6-25069f338a45
md"Then, we compute the overlap with all modes:"

# ╔═╡ 293510e1-336a-47d4-99fe-18d336cabff4
ol=overlap.(ms2D,f)

# ╔═╡ f5a31503-47f4-4f0c-9354-ff2d0e762d9e
md"The fraction of power transmitted in the fiber is (by neglecting Fresnel reflection):"

# ╔═╡ 9518861f-6b82-487a-96ad-2f78a1b4ede6
T=sum(abs2.(ol))

# ╔═╡ 43a98053-bace-40c8-8b3f-fae68cf2c4c3
md"The fraction of power transmitted in the funcamental mode is:"

# ╔═╡ 4d27b5b6-22d9-4aab-ae2f-da3209752ac9
T01=abs2(ol[1])

# ╔═╡ 78ec6f77-cb50-49e3-800e-0a26010ed8fd
md"One can find again this result with an analytical formula. Indeed, the transmission of a gaussian mode A with a waist $w_A$ to a gaussian mode B with a waist $w_B$ is: $T=\frac{4w_A^2w_B^2}{\left(w_A^2+w_B^2\right)^2}$"

# ╔═╡ 0d716824-f534-4e9f-9979-0438ff4fd073
md"The fundamental mode of the fiber can be approximated by a gaussian mode with a waist equal to the half of the modal field diameter (MFD)."

# ╔═╡ cc38bd13-c243-499c-a0e4-9f7d07c5d0b2
wm=MFD(ms2D[1])/2

# ╔═╡ e82934f6-bfde-48ba-ba71-3da03b01c7d0
T01_analytical=4*wf^2*wm^2/(wf^2+wm^2)^2

# ╔═╡ 9f40fd9e-f902-438b-b2af-e3c105684cbc
md"""
## Propagation in the fiber
To propagate a mode, just multiply it by $\exp(i\beta z)$
"""

# ╔═╡ 1e3cbba1-5448-4819-a6c8-de7de845981d
md"By symmetry, only the modes LP$_{01}$ (mode 1) and LP$_{02}$ (mode 4) are involved. The beating length (in µm) between these two modes is:"

# ╔═╡ 2cfb5868-2aae-475f-aede-1a44a4a76ac1
lambda/(ms2D[1].neff-ms2D[4].neff)

# ╔═╡ 2f6b424b-18e2-40a8-861e-6295b1f822d5
md"Propagation distance $z$"

# ╔═╡ 6789263d-a4a0-428a-b2d8-abb387cafa96
@bind z PlutoUI.Slider(0:78)

# ╔═╡ a6093e43-2e5f-4cdf-937f-47e46aef5067
fz=ol[1]*getField(ms2D[1],z)+ol[4]*getField(ms2D[4],z);

# ╔═╡ 9abf49f4-c615-4c6f-ba99-2825a3b80138
MFD(fz)

# ╔═╡ 3b4d33d2-70b9-4638-badd-025e1d560a91
plot(abs2(fz.E),axis=(aspect=DataAspect(),title="Intensity at distance z"))

# ╔═╡ 6e25b729-96a4-43d4-8886-5d83302251f0
md"""
## Exercise
We inject a gaussian beam with a waist $wf=2$ µm but the center of the beam is offset from the center of the fiber by a distance of $d=2.5$ µm. Compute the transmission in the fundamental mode and in the first higher order mode in dB.
"""

# ╔═╡ 999985c3-9d8d-4b64-86a2-a81840f75b9e
d=2.5

# ╔═╡ 71c5f6a6-e012-472e-898a-fd9dbde18a3a
...
manifest_format = "2.0"
project_hash = "d3d77c399016aca48bbd7e251d4d511dff598fa8"
...
git-tree-sha1 = "72b7eb03a098c79860f8676bf3959449285516f5"
...
git-tree-sha1 = "4b8a6b08f3aec65938e6983eb9df963933f1a294"
