...
# ╔═╡ 8d3e15a7-2118-400b-bffa-5b91f02a075e
using PlutoUI

# ╔═╡ ca7d5367-a80d-4e87-b0aa-46b420c0a457
using Plots, OpticalFibers,Gridap,OpticalFibers.ModeSolvers
...
md"""
# Gradient index fiber and computation of the group velocity with Finite Element Method
## Computation of scalar guided modes with the function `FEM1D` (fiber with rotational symmetry)
Parabolic refractive index core fiber with a maximal refractive index of 1.47 and a core radius of 2 µm. The cladding refractive index is 1.45. 
"""
...
if (isempty(ms))
	...
	md"Radial number `nmode` = $(@bind nmode NumberField(1:length(ms), default=1))"
end
...
begin
	if (@isdefined nmode)
		...
		plot!([minimum(r),maximum(r)],[ms[nmode].neff,ms[nmode].neff],linestyle=:dash,label="neff",legend=:top)
		plot!(twinx(),r,-real.(computeField(ms[nmode],r)),color=:green,label="Mode LP"*string(l)*","*string(nmode),xlims=(minimum(r),maximum(r)),legend=:right,ylabel="E",title="1D Plot")
	end
end

# ╔═╡ 6b698ddd-4db9-4a96-a08e-6cdfcf69465e
md"Symmetry of the mode:"

# ╔═╡ 4c4ad1f6-c9c1-4780-b25c-e0d7b96093bc
begin
	if (isempty(ms))
		md"No mode"
	else
		if l==0
			...
			@bind fun Select(["cos", "sin"])
		end
	end
end

# ╔═╡ ac4d7cf8-eb54-44de-b70a-6dbb8af4f8ef
begin
	if (@isdefined nmode)
		if (fun=="cos")
			f=cos
		...
		rr=-rmax/2:rmax/200:rmax/2
		contourf(rr,rr,-real.(computeField(ms[nmode],hypot.(rr,rr')).*f.(l*atan.(rr,rr'))),levels=100,linewidth=0,aspect_ratio=:equal,title="2D Plot - Mode LP"*string(l)*","*string(nmode),xlims=(minimum(rr),maximum(rr)),ylims=(minimum(rr),maximum(rr)),xlabel="x (µm)",ylabel="y (µm)")
	end
end
...
md"""
## Exercise 1:
Use the `FEM1D` function to find again the modes of the three layers fiber described in the `multi_step_fiber_modes` function below.
"""

# ╔═╡ 28ba7674-771d-4fd0-a543-9825873b53f7
ms2_step=multi_step_fiber_modes.(1,[0,1],Ref([0.5,2]),Ref([1.48,1.47,1.45]))

# ╔═╡ dcade700-d7aa-4588-b528-6b428eb26a95


# ╔═╡ 790eb6b8-9753-4b16-99c1-2c778c95beda
md"## Computation of the guided scalar modes with the function `FEM2D`"

# ╔═╡ bc883cf0-8b6e-40fe-9668-1b1dded0efbd
md"In this part, we use a mesh created with GMSH. The core radius `a` is fixed to 2 µm and the domain of integration is limited to r=10 µm. If you want to use other values, you have to modify the file `example1.geo` and generate a new mesh with GMSH."

# ╔═╡ a005da69-2a18-48a5-ac96-931a29ba0249
md"First, we load the mesh in memory:"

# ╔═╡ 87e030da-f31f-4f99-b52d-70e14ebbb16b
model2D = GmshDiscreteModel("example1.msh");

# ╔═╡ ddc08e2c-6765-4f03-9410-eb8cf47df361
md"The 2D permittivity is calculated from the 1D permittivity"
...
contourf(x,x,(sqrt.(epsilon2D.(tuple.(x,x'))))',linewidth=0,levels=100,title="RIP",aspect_ratio=:equal,xlims=(minimum(x),maximum(x)),ylims=(minimum(x),maximum(x)))
...
md"A mode can also be saved in a vtk file that can by read by paraview."
...
md"""
## Computation of effective area, Mode Field Diameter (MFD) and nonlinear coefficient
"""

# ╔═╡ 64ae25de-d340-4344-9f47-0f2f7044cd89
md"The effective area of a scalar mode is defined by $A_{\text{eff}}=\frac{\left( \iint \vert E \vert^2 dS \right)^2}{\iint \vert E \vert^4 dS}$"
...
md"The MFD is the beam width at $1/e^2$ in intensity"
...
md"For scalar modes, the nonlinear coefficient is defined by $\frac{n_2\omega}{cA_{\text{eff}}}$"
...
md"""
## Taking into account the material dispersion and computation of the group velocity
We will now consider the dispersion of the silica constituting the cladding of the previous fiber. As a first approximation, we will assume that the difference in core/cladding index does not vary with wavelength and is fixed at 0.02.
"""
...
# Computation of the fundamental Mode (FM)
ms_dispersif=FEM1D(lambda2,0,x->epsilon_dispersif(lambda2,x),model,field=true)

# ╔═╡ fa8afffa-6cbf-45ee-afe2-37cdb06c8509
begin
	r2=0:0.05:10
	plot(r2,sqrt.(epsilon_dispersif.(lambda2,r2)),xlabel="r (µm)",ylabel="Refractive index",label="RIP",xlims=(minimum(r2),maximum(r2)),legend=:top)
...
md"To plot the evolution of the effective index as a function of wavelength, it is necessary to create a vector of wavelegnths and the corresponding vector of permittivity functions."
...
md"""
## Exercise 2: Group velocity
Compute the group velocity and the group index of the LP$_{01}$ and LP$_{11}$ modes for the wavelength `vlambda[61]`=700 nm by making a discrete derivative.
"""

# ╔═╡ b3983b11-ac10-4806-b39f-70be6f44ae8e
begin
	vomega=2*pi*(PhysicalData.c)./(vlambda*1e-6) #vector of angular frequencies
	vbeta01=neff01*2*pi./(vlambda*1E-6) #propagation constant of the LP01 mode 
	vbeta11=neff11*2*pi./(vlambda[1:N11]*1E-6) #propagation constant of the LP11 mode
end;

# ╔═╡ 4fc3da9c-095b-4536-adae-be8400cc217d
...
manifest_format = "2.0"
project_hash = "d78dc46dd4c190542dacdfaf3252e0d1b5f0068d"
