...
# ╔═╡ d5049643-f1a0-48fc-8382-90eee3efd9e7
using OpticalFibers,Plots, PlutoUI

# ╔═╡ 7af97197-8e0a-45c2-baa3-8beb0894ddad
using OpticalFibers.ModeSolvers

# ╔═╡ 4349fa50-1c39-11ef-29b1-2d7b37538b15
md"""
# Step index fiber
## Silica rod in air
Computation of vector modes of a silica rod ( $n=1.45$ and diameter $2a=1.5$ µm) located in air ($n=1$) at a wavelength of 1 µm.
"""


# ╔═╡ 12ffcd1e-2fb3-49fb-91c7-23da2d78efe1
begin
...
# ╔═╡ 22188907-4b27-4e14-8277-1f8a5ec54f77
if V>2.405
	md"The normalized frequency `V` = $(round(V*1000)/1000), this is a multimode fiber"
else
	md"The normalized frequency `V` = $(round(V*1000)/1000), this is a single-mode fiber"
end

# ╔═╡ 6e4416b9-f8d6-4c2b-a04a-4c02ac59b993
md"Value of $\nu$ (azimuthal number) :"

# ╔═╡ e0561cd0-e476-4a45-a1ac-03ce241240cf
@bind mu NumberField(0:10, default=1)

# ╔═╡ ba9d9442-33e2-4097-a688-9ba5888498bf
m=multi_step_fiber_modes(lambda,mu,a,[nc,ng],field=true,type=:Vector)

# ╔═╡ dbdfbc1c-334c-4599-93a1-04e940849b0b
md"Value of $\mu$ (radial number) :"

# ╔═╡ c31f2822-afb7-4e3e-8a3c-febeeb767a55
if (isempty(m))
	md"Pas de mode"
else
	@bind nmode NumberField(1:length(m), default=1)
end

# ╔═╡ 751e841b-7c26-450c-828f-3c8301d2a74f
#Position vector for plots
x=a*(-3:0.05:3);

# ╔═╡ 889a51a5-8b1e-4e98-8c7f-7d73ea35ce8a
begin
	if (@isdefined nmode)
		p1=contourf(x,x,real(computeField(m[nmode],x,x',:Ex))',levels=50,linewidth=0,aspect_ratio=:equal,title="Ex");
		p2=contourf(x,x,real(computeField(m[nmode],x,x',:Ey))',levels=50,linewidth=0,aspect_ratio=:equal,title="Ey");
		p3=contourf(x,x,imag(computeField(m[nmode],x,x',:Ez))',levels=50,linewidth=0,aspect_ratio=:equal,title="Ez");
		p4=contourf(x,x,real(computeField(m[nmode],x,x',:Hx))',levels=50,linewidth=0,aspect_ratio=:equal,title="Hx");
		p5=contourf(x,x,real(computeField(m[nmode],x,x',:Hy))',levels=50,linewidth=0,aspect_ratio=:equal,title="Hy");
		p6=contourf(x,x,imag(computeField(m[nmode],x,x',:Hz))',levels=50,linewidth=0,aspect_ratio=:equal,title="Hz");
		plot(p1, p2, p3, p4, p5, p6, layout=(2,3), legend=false,size=(900,600),plot_title="Mode "*m[nmode].Name)
	end
end

# ╔═╡ 031121a0-9825-40bb-a53e-9e25e3bf7c71
begin
	if (@isdefined nmode)
		Px,Py,Pz=PoyntingVector(m[nmode]);
		contourf(x,x,Pz.(tuple.(x,x'))',linewidth=0,levels=100,title="Pz",aspect_ratio=:equal,xlims=(minimum(x),maximum(x)),ylims=(minimum(x),maximum(x)))
	end
end

# ╔═╡ 39a6c426-2f92-4e58-833c-f04cc36c0b3b
md"""
## Exercise 1: Ring core fiber
Compute the fundamental mode of a ring core fiber at the wavelength of 1.55 µm :
- Core: radius 3 µm and refractive index 1.4
- Ring: thickness 1 µm and refractive index 1.5
- Cladding: refractive index 1.2
"""

# ╔═╡ 0ffea69d-2d1c-4454-8eed-53d8ba6b74b7
#m2=..

# ╔═╡ 64a1e3a1-ad95-439a-a50b-45cfac6b51be
# ╠═╡ disabled = true
#=╠═╡
begin
	x2=(-8:0.1:8)
	pl1=contourf(x2,x2,real(computeField(m2[1],x2,x2',:Ex))',levels=50,linewidth=0,aspect_ratio=:equal,title="Ex");
	pl2=contourf(x2,x2,real(computeField(m2[1],x2,x2',:Ey))',levels=50,linewidth=0,aspect_ratio=:equal,title="Ey");
	pl3=contourf(x2,x2,imag(computeField(m2[1],x2,x2',:Ez))',levels=50,linewidth=0,aspect_ratio=:equal,title="Ez");
	pl4=contourf(x2,x2,real(computeField(m2[1],x2,x2',:Hx))',levels=50,linewidth=0,aspect_ratio=:equal,title="Hx");
	pl5=contourf(x2,x2,real(computeField(m2[1],x2,x2',:Hy))',levels=50,linewidth=0,aspect_ratio=:equal,title="Hy");
	pl6=contourf(x2,x2,imag(computeField(m2[1],x2,x2',:Hz))',levels=50,linewidth=0,aspect_ratio=:equal,title="Hz");
	plot(pl1, pl2, pl3, pl4, pl5, pl6, layout=(2,3), legend=false,size=(900,600),plot_title="Mode "*m2[1].Name)
end
  ╠═╡ =#

# ╔═╡ dd9a3331-ed3e-43e2-b4b3-80bbae17d1ff
# ╠═╡ disabled = true
#=╠═╡
begin
	Px2,Py2,Pz2=PoyntingVector(m2[1]);
	contourf(x2,x2,Pz2.(tuple.(x2,x2'))',linewidth=0,levels=100,title="Pz",aspect_ratio=:equal,xlims=(minimum(x2),maximum(x2)),ylims=(minimum(x2),maximum(x2)))
end
  ╠═╡ =#

# ╔═╡ 1072d77b-e21a-4589-b8c0-44f8e96d0e42
md"""
## Exercise 2: Weak guidance approximation
Study of a step index fiber
- Core: diameter 2 µm and refractive index 1.47
- Cladding: refractive index 1.45
At a wavelength of 1 µm, this fiber is bimodal (V=3).\
Compute the effective indices of all the scalar LP modes and all vector modes. Verify that the vector modes are quasi-degenerate in LP modes.
"""

# ╔═╡ eecf83c9-e193-4362-b32a-a5ac7dbfb6a3


# ╔═╡ 8048633f-c637-4a2d-859e-15c6bd21f6bf
...
manifest_format = "2.0"
project_hash = "6d9db793a5a7f265295b510a73cad80b849e4cc1"
